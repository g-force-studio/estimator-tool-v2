...
ALTER TABLE estimate_queue ENABLE ROW LEVEL SECURITY;

CREATE UNIQUE INDEX estimate_queue_job_active_unique
  ON estimate_queue(job_id)
  WHERE status IN ('pending', 'running');

CREATE INDEX estimate_queue_status_idx
  ON estimate_queue(status, created_at);

-- Extensions for hybrid search
CREATE EXTENSION IF NOT EXISTS pg_trgm;
CREATE EXTENSION IF NOT EXISTS vector;

-- Hybrid search columns for pricing materials
ALTER TABLE pricing_materials
  ADD COLUMN IF NOT EXISTS embedding vector(1536);

ALTER TABLE workspace_pricing_materials
  ADD COLUMN IF NOT EXISTS embedding vector(1536);

CREATE INDEX IF NOT EXISTS pricing_materials_item_key_trgm_idx
  ON pricing_materials USING gin (item_key gin_trgm_ops);

CREATE INDEX IF NOT EXISTS pricing_materials_aliases_trgm_idx
  ON pricing_materials USING gin (aliases gin_trgm_ops);

CREATE INDEX IF NOT EXISTS workspace_pricing_materials_norm_trgm_idx
  ON workspace_pricing_materials USING gin (normalized_key gin_trgm_ops);

CREATE INDEX IF NOT EXISTS workspace_pricing_materials_desc_trgm_idx
  ON workspace_pricing_materials USING gin (description gin_trgm_ops);

CREATE INDEX IF NOT EXISTS pricing_materials_embedding_idx
  ON pricing_materials USING ivfflat (embedding vector_cosine_ops);

CREATE INDEX IF NOT EXISTS workspace_pricing_materials_embedding_idx
  ON workspace_pricing_materials USING ivfflat (embedding vector_cosine_ops);

-- Dequeue the next estimate job (worker uses SKIP LOCKED for concurrency)
CREATE OR REPLACE FUNCTION dequeue_estimate_job()
RETURNS TABLE (
  id UUID,
  job_id UUID,
  workspace_id UUID,
...
CREATE OR REPLACE FUNCTION search_pricing_candidates(
  query_text TEXT,
  ...
  customer_rows AS (
    SELECT
      ...
      CASE
        ...
        ELSE 0
      END AS vector_score
    FROM workspace_pricing_materials wpm, params
  ...
  catalog_rows AS (
    SELECT
      ...
      CASE
        ...
        ELSE 0
      END AS vector_score
    FROM pricing_materials pm, params
